...
MODEL_PATH = os.path.join(SUBJECT_FOLDER, f'{SUB}_smooth_{SMOOTHING_FWHM}_{HEIGHT_CONTROL}_{CONTRAST_TYPE.value}_fitted_glm.pkl')
PRE_MODEL_PATH = os.path.join(SUBJECT_FOLDER, f'{SUB}_pre_smooth_{SMOOTHING_FWHM}_{HEIGHT_CONTROL}_{CONTRAST_TYPE.value}_fitted_glm.pkl')
POST_MODEL_PATH = os.path.join(SUBJECT_FOLDER, f'{SUB}_post_smooth_{SMOOTHING_FWHM}_{HEIGHT_CONTROL}_{CONTRAST_TYPE.value}_fitted_glm.pkl')
SECOND_LEVEL_MODEL_PATH = os.path.join(SUBJECT_FOLDER, f'{SUB}_second_level_smooth_{SMOOTHING_FWHM}_{HEIGHT_CONTROL}_{CONTRAST_TYPE.value}_fitted_glm.pkl')
...
if LOAD_MODELS:
    ...
    fmri_glm, contrast = combined_first_level_analysis(fmri_concat, events_concat, metadata_concat, confounds_concat, CONTRAST_TYPE, SMOOTHING_FWHM)
    ...
    pre_glm, pre_contrast = first_level_analysis(pre_fmri_data, pre_events_df, pre_metadata, pre_confounds_df, CONTRAST_TYPE, SMOOTHING_FWHM, title=f"Pre Session")
    post_glm, post_contrast = first_level_analysis(post_fmri_data, post_events_df, post_metadata, post_confounds_df, CONTRAST_TYPE, SMOOTHING_FWHM, title=f"Post Session")
