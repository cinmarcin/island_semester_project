...
# Compute F-statistic to observe whether any difference exists between repetitions
from scipy.stats import f_oneway
groups = [pre_avgs[:, r] for r in range(n_reps)]
f_stat, p_value = f_oneway(*groups)
print(f"ANOVA across repetitions (Pre): F={f_stat}, p={p_value}")

# same for post 
groups_post = [post_avgs[:, r] for r in range(n_reps)]
f_stat_post, p_value_post = f_oneway(*groups_post)
print(f"ANOVA across repetitions (Post): F={f_stat_post}, p={p_value_post}")
